...
struct CalculatorView: View {
    ...
    private func buttonHeight(_ geometry: GeometryProxy) -> CGFloat {
        // 사용 가능한 높이 계산 (일일 팁은 overlay이므로 고려하지 않음)
        let displayHeight: CGFloat = 200
        let topMargin: CGFloat = 20
        let verticalPadding: CGFloat = 20 // 버튼 영역 상단 패딩만
        let availableHeight = geometry.size.height - displayHeight - topMargin - verticalPadding - geometry.safeAreaInsets.bottom
        ...
        // 최소 높이만 보장 (최대 높이 제한 제거)
        return max(45, buttonHeight)
    }
